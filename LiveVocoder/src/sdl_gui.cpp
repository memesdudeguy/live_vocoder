...
int pa_callback(const void* in, void* out, unsigned long frames, const PaStreamCallbackTimeInfo*,
    ...
    // Mute speakers only if monitor is off AND we are NOT routing to a virtual (null-sink) output.
    // When using live_vocoder sink (Wine or native), we always want to keep feeding the virtual mic.
...
std::filesystem::path documents_livevocoder_dir() {
    ...
    // Wine launched from Linux often has HOME but not USERPROFILE; map ~/Documents → Z:/home/…/Documents
...
int run_sdl_gui(char* argv0, const char* carrier_path_opt) {
    ...
    auto sync_virt_sink_speaker_loopback = [&]() {
        auto pulse_sink_for_monitor = []() -> const char* {
            ...
            if (n > 0 && n < sizeof(s_pulse_sink_buf)) {
        ...
        // Wine monitor-off while streaming: host script unloads loopbacks and moves LiveVocoder.exe to the null sink.
        // After Stop (streaming false): only unload loopbacks — no long move-retry loop.
        auto mute_speaker_monitor = [&](bool wine_move_retries) {
            lv_linux_sync_speaker_monitor_loopback(false, nullptr, wine_move_retries);
        };
#if defined(__linux__)
...
#endif
        if (!streaming) {
            mute_speaker_monitor(false);
            sync_linux_physical_monitor_mute();
            return;
        }
        if (!app.monitor_on.load(std::memory_order_relaxed)) {
            mute_speaker_monitor(true);
            sync_linux_physical_monitor_mute();
            return;
        }
        if (!app.pulse_virt_sink_output.load(std::memory_order_relaxed)) {
            // Direct speakers/headphones: hear via PortAudio — do not unload PipeWire loopbacks (other apps / user routes).
            sync_linux_physical_monitor_mute();
            return;
        }
            ...
            if (e.type == SDL_QUIT) {
                ...
                if (hh == 0) {
                    ...
                    if (ioh == 0) {
                        ...
                        app.monitor_on.store(!app.monitor_on.load(std::memory_order_relaxed), std::memory_order_relaxed);
                        if (streaming) {
                            // Loopback + Linux physical sink-input mute stay in sync via one path.
                            sync_virt_sink_speaker_loopback();
                        }
                        refresh_title();
                    } else {
