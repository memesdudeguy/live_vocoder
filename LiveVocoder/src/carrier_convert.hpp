...
#if defined(_WIN32)
/**
 * SDL drag-and-drop and some launchers pass POSIX ``/home/...`` or ``file:///...`` paths; under Wine, Win32
 * ``std::filesystem`` and ``ffmpeg.exe`` need ``Z:/home/...``. Drive paths (``C:\``) are unchanged.
 */
