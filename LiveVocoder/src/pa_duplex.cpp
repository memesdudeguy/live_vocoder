...
#if defined(_WIN32)
    // Wine: registry keys set by Inno Setup (HKCU\Environment)
    HKEY hKey;
    if (RegOpenKeyExA(HKEY_CURRENT_USER, "Environment", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
        char buf[64];
        DWORD sz = sizeof(buf);
        if (RegQueryValueExA(hKey, "PULSE_SINK", NULL, NULL, (LPBYTE)buf, &sz) == ERROR_SUCCESS ||
            RegQueryValueExA(hKey, "LIVE_VOCODER_PULSE_SINK", NULL, NULL, (LPBYTE)buf, &sz) == ERROR_SUCCESS) {
            RegCloseKey(hKey);
            return true;
        }
        RegCloseKey(hKey);
 ...
 *  Wine exposes "Speakers (PulseAudio Output)" — also a PulseAudio route when PULSE_SINK is set. */
static bool pa_output_device_is_pulse_pcm_virt_route(const char* name) {
    ...
    if (lv_ci_hay_contains(name, "pulseaudio") && lv_env_pulse_sink_targets_virt_route()) {
        return true;
    }
    return false;
}
...
std::string pa_portaudio_virt_capture_hint() {
    ...
    return "PortAudio capture looks like virtual route: " + hits[0];
}
