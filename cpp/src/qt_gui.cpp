        ...
        if (!carrier_file_is_raw_f32_carrier(cp)) {
            ...
            if (!carrier_ffmpeg_to_f32(lv_gui::kSampleRate, cp, dest, conv_err)) {
                QMessageBox::critical(
                    this, QStringLiteral("Live Vocoder — carrier"),
                    QStringLiteral("Could not convert carrier to .f32 (see README.txt next to the app):\n\n%1")
                        .arg(QString::fromStdString(conv_err)));
                return false;
            }
...
int run_qt_gui(int argc, char** argv) {
    ...
    QApplication::setOrganizationName(QStringLiteral("memesdudeguy"));
