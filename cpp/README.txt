...
**Minimal C++ only** (exe + DLLs, no Python): ``LiveVocoder-Setup.exe`` — unified **x64 + ARM64** Windows installer from CI (local cross-build stages **x64** only under ``dist-windows-installer-minimal/x64/`` unless you set ``LV_MINIMAL_ARM64_DIR``). **Wine on Linux/macOS** via
...
**qpwgraph / Helvum under Wine:** The minimal installer’s ``live-vocoder-wine-launch.sh`` (and repo ``run-wine.sh`` / ``cpp/run-wine-cross-exe.sh``) pass Pulse’s official dotted keys via ``env`` — ``PULSE_PROP_application.name``, ``PULSE_PROP_media.name``, ``PULSE_PROP_application.icon_name`` — because Bash cannot ``export`` names that contain dots. That should label streams **Live Vocoder** instead of **LiveVocoder.exe [audio stream…]** (restart the app after updating the script). Duplex still appears as **two** stream nodes (playback + capture); a **non-pw** icon is normal for **PulseAudio-protocol** clients. Override names: ``LIVE_VOCODER_PULSE_APP_NAME``, ``LIVE_VOCODER_PULSE_MEDIA_NAME``, ``LIVE_VOCODER_PULSE_ICON_NAME``. Re-run the minimal installer to refresh an old ``live-vocoder-wine-launch.sh``.

**SDL welcome under Wine:** The installer’s ``live-vocoder-wine-launch.sh`` defaults ``LIVE_VOCODER_SDL_SKIP_STARTUP_MODALS`` to **1**, so the SDL tips / font-info boxes do not open every run. Set ``LIVE_VOCODER_SDL_SKIP_STARTUP_MODALS=0`` to show them. Repo ``run-wine`` scripts do not set this unless you export it (see **Automation / headless SDL** above).
...
-------------------------------------------------------
  ...
  **default input/output** devices — the same defaults as the Sound control panel.
  ...
  Minimal: cpp/build-installer-minimal.sh → LiveVocoder-Setup.exe (stage ``dist-windows-installer-minimal/`` first)
