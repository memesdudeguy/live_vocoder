...
if not exist "%SRC%\LiveVocoder.exe" (
  echo Missing %SRC%\LiveVocoder.exe — run build-cross-windows with LV_BUNDLE_PYTHON=0 or bundle DLLs. >&2
  exit /b 1
)
...
>>"%README%" echo Non-.f32 carriers need ffmpeg.exe next to the app or ffmpeg on PATH.
