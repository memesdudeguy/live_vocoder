...
QEMU_ARGS+=(-drive "file=$DISK,if=none,id=nvme0,format=qcow2")
...
echo "Disk: $DISK  |  -display $QEMU_DISPLAY_SPEC  |  RAM: ${RAM_MB}M  |  audio: $AUDIO_SUMMARY  |  smbd: $(command -v smbd 2>/dev/null || echo none)" >&2
"$QEMU_BIN" "${QEMU_ARGS[@]}"
rc=$?
cleanup_swtpm
trap - EXIT
