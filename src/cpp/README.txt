...
**Minimal C++ only** (exe + DLLs, no Python): ``LiveVocoder-Setup.exe`` via
``build-installer-minimal.sh`` / ``build-installer-minimal.bat`` and ``installer/LiveVocoderCppMinimal.iss`` (single ISCC pass; Wine-only behavior uses ``Check: IsRunningUnderWine``).
